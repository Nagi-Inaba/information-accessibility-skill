...
## Inspection Intake And Completion

Before inspecting a target, establish the intended use of the report and select `quick` (簡易チェック) or `detailed` (詳細検査・改善用). An explicit request for a brief/basic check selects quick; a request for implementation-ready findings selects detailed. Reuse a clear choice already made in the conversation. Do not ask again just because the wording differs. When neither intent is clear, ask once: 「主要な問題を短く把握する簡易チェックと、修正に使う詳細検査のどちらを希望しますか？」 Do not begin target inspection until that choice is available; prepare the scope from supplied information meanwhile.

State the selected level, report purpose, included/excluded pages or screens, interactions/states, environment, and expected deliverables before starting. Resolve consequential missing scope information without requiring a large intake form. Use the existing run scope/environment fields for the agreed boundaries. Initialize with `--inspection-mode quick|detailed --inspection-purpose <requested-use>`; the current run stores `inspection_request` with the selected mode's deliverables and completion criteria. These options neither authorize target changes nor raise the evidence level. An old run without this field has no recorded level; do not relabel it or invent a choice.

- **Quick:** inspect the named page's representative state using available rendered DOM/accessibility-tree checks, a basic keyboard path, and narrow-screen reflow. List the main barriers in priority order and state what was not inspected. Do not expand into additional journeys or environments. Use a short summary; preserve the complete underlying profile record.
- **Detailed:** agree the pages, states, complete interactions, and environments first. Exercise the supported read-only checks across that scope, including applicable state transitions, focus return, text resize, skip-link destination/next focus, and content meaning checks. For each finding, record its location, reproduction steps and evidence in the observation/issue, affected users, a concrete change, and a retest procedure. Name any unavailable environment or human check and its next step. Deliver a decision summary with remediation details and a full result appendix.

Before reporting the inspection as complete, compare every `inspection_request.completion_criteria` entry with the registered evidence and delivered sections. Report which conditions are met with evidence references and which remain unmet with the next action. A populated template, displayed checklist, or generated file is not evidence that a condition was met. If agreed operations/environments were not covered or a finding lacks actionable details, call the delivery partial; do not silently lower the selected level. AI evidence remains E0/E1 in both modes. Profile conformance and authorized source changes retain their separate existing boundaries. `--detail summary|full` changes presentation length only.
...
- Exercise the read-only checks agreed for the selected inspection level against the real target and register target-specific E0/E1 `SCREEN-*` evidence. Quick mode does not imply that all 55 requirements were inspected. Keep machine-readable profile rows at `mapping_status: "unverified"` and `outcome: "not_tested"` unless a separate external human result exists.
- Project the evidence actually inspected into the report vocabulary `適合`, `不適合`, `要確認`, and `未確認`. List `not_applicable` separately with a concrete rationale. For `未確認`, distinguish an unavailable test from a check outside the agreed inspection depth, and name the next test or evidence required.
...
1. Complete Inspection Intake above, then fix the target identity, exact `version_or_commit`, profile, included and excluded scope, complete processes, third-party content, environment, and permissions. Create a new versioned `audit-run` with `create-audit-run.mjs`, recording the level and purpose; operate only on the schema version declared current by the installed `orchestration-registry.json` and never overwrite a prior run.
