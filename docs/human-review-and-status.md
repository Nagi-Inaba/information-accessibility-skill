...
日付は実在する `YYYY-MM-DD` を使います。うるう年は400年・100年・4年の規則で判定します。`2026-02-30` などは拒否し、翌月へ自動補正しません。

現行のassessment、run、history、screening、人手レビュー、修正承認・変更記録、証拠参照、対象snapshotは、`2026-09-18T09:00:00+09:00` または `2026-09-18T00:00:00Z` のようなRFC 3339日時を受理します。入力のoffsetと小数秒をそのまま保持します。`T`／`Z`は小文字も受理します。タイムゾーンを省略した日時、時刻の24時表記、うるう秒の `:60` は対応外で、エラーにします。

履歴・成果物の前後関係と修正処理の期限は、UTC上の時点と小数秒で比較します。同じ時点の異なるoffset表記は同値です。DST切替時も入力されたoffsetから順序を判定し、地域のタイムゾーンや壁時計の時刻から推測しません。元の成果物やhashを正規化のために書き換えることはありません。CLIが新しく生成する日時は従来どおりUTCです。

固定済みの旧schemaはUTC `Z` の契約を維持します。旧schemaの日時にも実在日付の検証を適用するため、旧コードで桁数だけを満たしていた不存在日付は無効になります。その場合も原本を保存し、正しい日時を確認した新しい記録を作成してください。旧記録のhashや署名を直接修正しないでください。
