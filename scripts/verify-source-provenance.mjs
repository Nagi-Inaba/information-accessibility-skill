...
import { loadSourceProvenance, verifySourceProvenance, thirdPartyNoticeMarkdown } from "../codex/skills/information-accessibility-practice/scripts/lib/source-provenance.mjs";
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
export function verifySourceNotices(packageRoot = root) {
  const skillRoot = path.join(packageRoot, "codex/skills/information-accessibility-practice");
  ...
  for (const file of ["THIRD_PARTY_NOTICES.md", "codex/skills/information-accessibility-practice/references/third-party-notices.md", "claude/skills/information-accessibility-practice/references/third-party-notices.md"]) {
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (process.argv[2] === "--write-notices") {
      ...
      for (const file of ["THIRD_PARTY_NOTICES.md", "codex/skills/information-accessibility-practice/references/third-party-notices.md"]) fs.writeFileSync(path.join(root, file), notice, "utf8");
