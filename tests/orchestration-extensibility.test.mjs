...
test("frozen orchestration contracts retain their normalized hashes across line-ending conventions", () => {
  ...
  assert.equal(readJson(path.join(references, "orchestration-registry.json")).schema_version, "15.0.0");
  assert.equal(readJson(path.join(references, "audit-run.schema.json")).properties.schema_version.const, "16.0.0");
