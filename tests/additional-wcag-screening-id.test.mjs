...
test("all jp-public-web requirement IDs are accepted for report projection", () => {
  ...
  for (const requirementId of profile.requirement_ids) {
    const errors = [];
    validateJsonSchema(payload(requirementId), schema, "$", errors);
    assert.deepEqual(errors, [], `${requirementId}: ${errors.join("; ")}`);
  }
});
