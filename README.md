...
検査を始める前に、レポートを何に使うかと、必要な検査の深さを決めます。会話で指定済みなら引き継ぎ、曖昧な場合だけ簡易・詳細の希望を確認します。

| レベル | 使いどころ | 受け取る内容 |
| --- | --- | --- |
| 簡易チェック（`quick`） | 大きな問題を把握し、次の検査や改善を判断したい | 指定範囲の基本検査、主要な問題と優先度、根拠、未確認事項と次の確認 |
| 詳細検査・改善用（`detailed`） | 担当者が具体的な修正に着手できる情報がほしい | 合意した画面状態・操作・環境の検査、指摘ごとの箇所・再現手順・影響・修正案・再検査方法、全項目の結果 |

新しいrunでは、`init`に`--inspection-mode quick|detailed --inspection-purpose "レポートの利用目的"`を指定します。対象範囲と環境は既存の`--config`で記録できます。レポートには選択内容と完了条件を表示し、実施記録で満たせない条件は未完了として次の作業を示します。レポートの表示量を変える`--detail summary|full`とは別の指定です。過去のrunは従来の内容のまま読めます。

## 詳細ドキュメント

- [人手レビューの記録・主張範囲・監査の状態確認](docs/human-review-and-status.md)
- [機能の提案・不具合・進捗](https://github.com/Nagi-Inaba/information-accessibility-skill/issues)
