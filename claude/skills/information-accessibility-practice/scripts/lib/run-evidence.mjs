...
export function createRunEvidenceReference({ run, targetRef, ...options }) {
  ...
  const measuredRun = ["8.0.0", "9.0.0", "10.0.0", "11.0.0"].includes(run.schema_version);
...
export function evidenceBindingErrors(reference, run) {
  ...
  try {
    ...
    if (["8.0.0", "9.0.0", "10.0.0", "11.0.0"].includes(run.schema_version)) {
...
export function collectScreeningEvidence(run, artifacts, readEvidence) {
  ...
  for (const artifact of artifacts) {
    if (artifact?.artifact_type !== "screening-observations" || artifact.payload?.schema_version !== "3.0.0") continue;
