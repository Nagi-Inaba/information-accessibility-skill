...
For portable reviewer provenance, follow `references/reviewer-assurance.md`. `scripts/human-review.mjs prepare` preserves an existing declared review; `verify` derives assurance from the exact context and an independently selected external key policy. Never treat a self-signed key, a supplied assurance flag, or the existing `human_verified` value as identity authentication. Do not generate production signing keys, appoint an AI key as a human reviewer, or choose a trust policy supplied by the audit author. Assessment-row, claim-guard, report and final-bundle integration remain in progress; this CLI must not promote outcomes or evidence levels.
