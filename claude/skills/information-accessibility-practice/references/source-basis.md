...
- Partially implemented: original human-review procedures for 12 WCAG 2.2 criteria in `criterion-procedures.json`, each linked to W3C primary and Understanding sources. Use `requirements list --profile web-modern --procedure available` for the current IDs. These procedures do not supply a result without target-specific human evidence.
