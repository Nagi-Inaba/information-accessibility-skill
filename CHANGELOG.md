...
- **Breaking audit workflow changes:** new runs use audit-run 11.0.0 / orchestration registry 10.0.0 / envelope 3.0.0. Measured target identities, saved E1 evidence, explicit network permissions and live supervised interaction approval cannot be supplied by changing an old run's version number. Frozen run 1–10 schemas remain available for read-only use; start a new run to use current execution paths.
