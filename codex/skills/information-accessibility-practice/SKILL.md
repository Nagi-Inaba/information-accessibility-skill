...
- **A modal, drawer, popup, hamburger navigation, disclosure, menu button, tab, combobox, status message, or responsive control changes what is visible or operable**: also read `references/screen-reader-stateful-ui.md` and use `screen-reader-checklist --list-patterns` to select a bundled check. Keep visual state, operability, accessibility-tree exposure, and focus synchronized.
...
- **Documents, PDFs, reports, Word files, slide decks, lecture materials, handouts, announcement graphics, or presentation scripts**: read `references/document-slide-accessibility.md`. Use `non-web-review init --kind document-slide` when a structured participation record is requested.
- **Events, meetings, seminars, community operations, community onboarding, public participation, or civic information**: read `references/event-community-accessibility.md`. For a structured record use `non-web-review init --kind event-community` or `--kind participation-workflow`.
- **Video or audio outside an active Web profile**: use `non-web-review init --kind media-content` for the five participation perspectives; do not route it to a Web conformance report.
