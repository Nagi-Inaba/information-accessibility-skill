...
Current run 10.0.0 requires `permissions.interaction_policy`. It is `null` for `read_only`. `human_supervised` requires the concrete policy below. Creating or reading this JSON records a proposed supervision scope; it does not establish live permission, authenticate a person, or authorize source writes. Run 9 and earlier remain read-only under their frozen schemas.
