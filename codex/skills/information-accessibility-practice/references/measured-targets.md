...
Current run schema 12.0.0 fixes a measured target inventory before the first artifact is registered. Envelope 3.0.0 records the exact inventory snapshot IDs. Registration remeasures the targets and rejects drift before adding evidence. Run 11 and earlier retain their frozen read contracts; create a new run for current registration.
