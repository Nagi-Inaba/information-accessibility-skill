...
After a completed change, `create-audit-run.mjs --supersedes-run <old-run.json>` creates a fresh retest run only from a valid supported predecessor (run 5, 6, 7 or 8) in `retest_required`. The new run must use a different run ID, target version, and empty non-overlapping artifact root while preserving the target, profile, references, scope and any recorded inspection request. Older predecessors without a request require an explicit level and purpose. It starts at `initialized` with source writes denied and copies no prior artifacts, history, outcomes, evidence or measured inventory. Capture and bind the new target state before retest observations.
