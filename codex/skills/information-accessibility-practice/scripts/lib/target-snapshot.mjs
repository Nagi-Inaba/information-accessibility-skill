...
export function createFileTargetSnapshot({ snapshotId, relativePath, bytes, version = null, capturedAt }) {
  ...
  return { ...common(snapshotId, capturedAt, "file"), identity: { relative_path: relativePath, version }, content_sha256: digest(bytes) };
}
...
export function createGitTargetSnapshot({ snapshotId, repository, commitSha, subpath = ".", capturedAt }) {
  ...
  return { ...common(snapshotId, capturedAt, "git"), identity: { repository, commit_sha: commitSha, subpath }, content_sha256: null };
}
