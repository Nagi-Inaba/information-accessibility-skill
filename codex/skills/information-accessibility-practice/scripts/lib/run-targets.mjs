...
function assertNetworkPermission(run, specs, options) {
  ...
  if (specs.some((spec) => spec.kind === "http")) {
    if (run.schema_version !== "16.0.0") throw new Error("HTTP target checks require a current run with a concrete network policy.");
...
export function targetBindingErrors(run, artifacts = []) {
  if (!["8.0.0", "9.0.0", "10.0.0", "11.0.0", "12.0.0", "13.0.0", "14.0.0", "15.0.0", "16.0.0"].includes(run?.schema_version)) return [];
  const errors = [];
