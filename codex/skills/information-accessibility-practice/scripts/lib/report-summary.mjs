...
export function renderReportSummaryMarkdown(presentation) {
  ...
  const lines = [
    ...
    ...Object.entries(presentation.audit_context?.participation_coverage ?? {})
      .map(([key, outcome]) => `- ${key}: ${escapeCell(outcome)}`),
    `- ${presentation.locale === "ja" ? "次回確認日" : "Next review date"}: ${escapeCell(presentation.audit_context?.next_review_at ?? text.none)}`,
    ...(presentation.audit_context?.next_review_condition ? [`- ${presentation.locale === "ja" ? "再確認条件" : "Review condition"}: ${escapeCell(presentation.audit_context.next_review_condition)}`] : []),
    ...(presentation.audit_context?.next_review_owner ? [`- ${presentation.locale === "ja" ? "担当者" : "Owner"}: ${escapeCell(presentation.audit_context.next_review_owner)}`] : []),
    `- ${presentation.locale === "ja" ? "独立監査の申告" : "Independent audit declared"}: ${presentation.audit_context?.independent_audit_performed ? (presentation.locale === "ja" ? "はい" : "Yes") : (presentation.locale === "ja" ? "いいえ" : "No")}`,
    ...(presentation.audit_context?.independent_audit?.scope_method ? [`- ${presentation.locale === "ja" ? "監査範囲と方法" : "Audit scope and method"}: ${escapeCell(presentation.audit_context.independent_audit.scope_method)}`] : []),
    ...(presentation.audit_context?.independent_audit?.report_location ? [`- ${presentation.locale === "ja" ? "監査報告の所在" : "Audit report location"}: ${escapeCell(presentation.audit_context.independent_audit.report_location)}`] : []),
